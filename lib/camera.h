...
class camera {
  ...
  private:
    ...
    double pixel_sample_scale; // Scale factor for averaging samples per pixel
    vec3   u, v, w;
    ...
    ray get_ray(int pixel_i, int pixel_j) {
        // Randomly sample within the pixel
        auto offset = sample_square();
        auto pixel_sample = pixel00_loc
                          + (pixel_i + offset.x()) * pixel_delta_u
                          + (pixel_j + offset.y()) * pixel_delta_v;
